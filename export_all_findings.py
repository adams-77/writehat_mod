...
def generateExcel(CVSSEngagementFindings, DREADEngagementFindings, ProactiveEngagementFindings, CVSSDatabaseFindings, DREADDatabaseFindings, ProactiveDatabaseFindings):
    ...
    columns = ['name','category','CWE','cvss_score','impacts','description','affectedResources','proofOfConcept','remediation','toolsUsed','vector','severity','cvssScore','references','engagement']
    row_num = 1
   ...
   # Iterate through ProactiveEngagementFindings
    ...
    # Define the titles for columns
    columns = ['name','category','CWE','cvss_score','impact','remediation','toolsUsed','vector','severity','cvssScore','references']
    row_num = 1
